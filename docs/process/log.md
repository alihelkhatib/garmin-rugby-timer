...
- 2026-04-07: Added a dedicated runtime integrity layer (`RugbyMatchIntegritySupport`) after repeated idle/halftime regressions showed that startup restore, input handling, rendering, update ticks, and persistence were all capable of observing partially invalid timer/state combinations. The app now reconciles the model to a small set of hard invariants before those paths proceed: idle always renders from `halfDuration`, halftime break owns only the break countdown, live half starts always reseed countdown state from configuration, invalid/unknown states fall back to sane idle, and idle/ended states clear any saved live snapshot instead of persisting pre-kickoff setup as resumable match state.

- 2026-04-07 match-integrity hardening validation: `./scripts/validate-local.sh` -> BUILD SUCCESSFUL for the app target (`bin/garminrugbytimer.prg`) and the unit-test target (`bin/tests.prg`).
