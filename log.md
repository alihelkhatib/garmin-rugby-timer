...
- Implemented all 17 source-change tasks for `018-multi-match-session` branch:
  ...
  - `source/RugbySettings.mc`: added "Clear Session" item (`SettingsMenu.initialize()`) and `:clear_session` handler (`RugbySettingsMenuDelegate.onSelect()`) that calls `RugbyTimerPersistence.clearSession()`.
