...
`
 - Log every build command in log.md (include simulator/device details). There is no configured repository CI workflow; use the local build scripts in `scripts/` to produce PRG artifacts and record the exact commands and simulator/device targets in `log.md`.
