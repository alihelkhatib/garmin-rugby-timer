...
- The top scoreboard is a measured header band with the gray elapsed timer above the scores, smaller `HOME` / `AWAY` support labels, primary score digits, per-team tries rendered beside their respective score columns, and a smaller centered half row below the score band.
...
- Card timers render in simple home/away columns under each score lane. The main screen now prioritizes the nearest-expiring timed sanction per team; additional timed cards or permanent-red markers are subordinate and may be omitted when space is tight.
