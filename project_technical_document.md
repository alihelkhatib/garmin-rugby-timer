...
- The top scoreboard is a measured header band with separate lanes for elapsed timer, team labels, score digits, and center metadata. The half/tries metadata now sits below a dedicated score band instead of inside the score digits, which keeps the round layouts readable without letting the center metadata collide with the score row.
