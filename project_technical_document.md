...
- The top scoreboard is a measured header band with the gray elapsed timer above the scores, smaller `HOME` / `AWAY` support labels, primary score digits, and optional compact-round metadata. On `compact_round`, the renderer now uses the smallest team-label tier, anchors tries directly from the score columns, and only restores elapsed time, then half, then tries when the measured layout still leaves the score/countdown lanes clean.
- `candidateTimerY` is the preferred vertical anchor for the large countdown once visible card rows are accounted for. The renderer clamps the final `countdownY` between the measured header/card handoff and the reserved lower state/hint boundary inside the safe content rect, and on tight compact-round screens it prefers “stay on screen” over preserving an oversized gap.
...
- The score band includes explicit `HOME` / `AWAY` labels above the score digits with team-distinct colors, but those labels are now smaller support text so color and position do more of the quick-glance work.
- Card timers render in simple home/away columns under each score lane. The main screen now prioritizes the nearest-expiring timed sanction per team; on compact round, a timed sanction forces `critical-only` layout mode so countdown/scores stay readable, and permanent-red markers are subordinate until no timed sanction remains.
