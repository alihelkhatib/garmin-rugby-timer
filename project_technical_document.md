...
- The top scoreboard is a measured header band with the gray elapsed timer above the scores, smaller `HOME` / `AWAY` support labels, primary score digits, and optional compact-round metadata. On `compact_round`, the renderer now steps through explicit detail modes so elapsed time returns before half, and half returns before tries only when the measured layout still leaves the score/countdown lanes clean.
